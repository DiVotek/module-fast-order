...
use function App\Filament\Resources\OrderResource\Pages\gettype;
...
class ViewFastOrder extends ViewRecord
{
    ...
    public function infolist(Infolist $infolist): Infolist
    {
        ...
        $orderHistories = DB::table(Order::HISTORY_TABLE)->where('order_id', $infolist->record->id)->get();
        $productOptions = [];
        $productsTable = [];
        ...
        return $infolist
            ->schema([
                ...
                Section::make(__('Delivery'))
                    ->schema([
                        KeyValueEntry::make('delivery_method')
                            ->columnSpanFull()
                            ->hiddenLabel()
                            ->keyLabel(__('Delivery info'))
                            ->valueLabel(false)
                            ->getStateUsing(function ($record) {
                                return [
                                    __('Delivery method') => __(Order::DELIVERY_METHODS[$record->details['deliveryMethod']]),
                                    __('Country') => $record->details['country'],
                                    __('City') => $record->details['city'],
                                    __('Address') => $record->details['address'],
                                    __('ZIP code') => $record->details['zip'],
                                ];
                            }),
                    ])->collapsible(),
                Section::make(__('Payment'))
                    ->schema([
                        KeyValueEntry::make('payment_method')
                            ...
                            ->getStateUsing(function ($record) {
                                return [
                                    __('Payment method') => __(PaymentMethod::PAYMENTS[$record->details['payment']]),
                                    __('VAT') => $record->details['vat'],
                                ];
                            }),
                    ])->collapsible(),
